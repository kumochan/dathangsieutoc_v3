...
var CommonTool = function() {
    ...
    this.loadOptionCategory = function(){
        if(catalog_scalar_url){
            ...
            chrome.runtime.sendMessage({ action: "getCategory", url: catalog_scalar_url, data: data_cate, method: 'POST', callback: 'afterGetCategory' });
        }
    };
    ...
    this.sendAjaxToCart = function (add_cart_url,data) {
        setTimeout(function(){
            ...
            data.action = add_to_cart_action;
    ...
    this.alert = function(){
        ...
        try{
            ...
            if(creditLevel >= 1 && creditLevel <= 7){
                ...
                text += 'Người bán này có uy tín bán hàng thấp. Quý khách nên cân nhắc trước khi đặt hàng.</a>';
...
var alibaba = function (cart_url,add_cart_url) {
    ...
    this.alert = function(){
        ...
        try{
            ...
            if(!star){
                text_info = 'Người bán này có uy tín bán hàng thấp. Quý khách nên cân nhắc trước khi đặt hàng.</a>';
            }
    ...
    this.getDataSend = function(item_data){
        try {
            ...
            if(!this.checkHasOneProperty()){
                try{
                    var item_price = item_data[4] ? item_data[4] : this.getPrice(item_data[0]);
...
var Action = {
    afterGetExchangeRate: function(request){
        ...
        if(request.response){
            var exchange = JSON.parse(request.response);
            exchange_rate = parseFloat(String(exchange.Data.CurrencyRate).replace(".", "").replace(",", ".")).toFixed(0);

...
class RoleController extends Controller
{
        ...
        try {
           ...
           // dd( $request->input('list-permission') );
            ...
            if ($update) {
                ...
                $flg = $role->update();
                $role_id = $role->id;
                // lay danh sach user de them quyen
                $userId = DB::table('users_roles')->where('role_id', $role_id)->get();
                ...
                foreach ($arr_permissions as $permission) {
                    if($permission != '')
                    {
                        ...
                        $rolePermission->permission_id = $permission;
                        $flg = $rolePermission->save();
                        //04. xoa du lieu bang user_permission
                        UserPermission::where('permission_id', $permission)->delete();

                        // 05. them du lieu bang user_permissions
                        $userId->each(function($item) use ($permission) {
    ...
    public function delete(Request $request)
    {
        ...
        try {
            ...
            DB::table('users_roles')->whereIn('role_id', $ids)->delete();
            // 02. Xoa du lieu bang roles_permissions
            DB::table('roles_permissions')->whereIn('role_id',$ids)->delete();

...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <form method="post" class="form-add"
              action="{{ action('Backend\RoleController@edit', array('id'=>$role->id)) }}">
            @csrf
            <div class="row">
                                        ...
                                        <ul role="group" class="jschildren">
                                            ...
                                            @foreach ($childPermissions as $subitem)
                                                <li role="treeitem" data-jstree="{&quot;opened&quot;:true}"
                                                    ...
                                                    @if (array_key_exists('hasChild', (array)json_decode($subitem->metadata))) {
                                                    <ul role="group" class="jschildren">
                                                        @php($childPermissions = \App\Permission::getAll(false,$subitem->thing_id))
                                                        @foreach ($childPermissions as $subitem_3)
                                                            <li role="treeitem"
                                                                data-jstree="{&quot;opened&quot;:true}"
                                                                aria-selected="false"
                                                                aria-level="2" aria-labelledby="j1_2_anchor"
                                                                aria-expanded="true"
                                                                id="{{$subitem_3->id}}"
                                                                ...
                                                                <i class="jstree-icon jstree-ocl"
                                                                   role="presentation"></i>
                                                                <a class="jstree-anchor" href="#" tabindex="-1"
                                                                   id="j1_2_anchor-{{$subitem_3->id}}">
                                                                    <i class="jstree-icon jstree-checkbox jstree-undetermined"
                                                                       role="presentation"></i>
                                                                    <i class="jstree-icon jstree-themeicon fa fa-folder jstree-themeicon-custom"
                                                                       role="presentation"></i>{{$subitem_3->name}}
                                                                </a>
                                                            </li>
                                                        @endforeach
                                                    </ul>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var getPermissionList = function (evt, data) {
                ...
                //var selected_parent = data.instance.get_node('1').id;
                var selected_parent = data.node.parents[0];
